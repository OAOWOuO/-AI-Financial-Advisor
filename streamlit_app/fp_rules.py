...
class RulesEngine:
        ...
        if months < min_m:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Only {months:.1f} months of expenses are covered by liquid assets. "
            ...
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Currently {months:.1f} months covered. "
    ...
    def _check_debt(self, p: ClientProfile) -> List[PlanningIssue]:
        ...
        consumer_burden = calc.calc_consumer_debt_burden(p.liabilities.consumer_total(), p.total_annual_income())
        ...
        if dti > r["max_total_dti"]:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Total DTI of {dti*100:.1f}% exceeds the maximum threshold of "
            ...
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"DTI is {dti*100:.1f}%, above the recommended {r['recommended_dti']*100:.0f}%. "
        ...
        if p.liabilities.credit_cards > 0:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Carrying ${p.liabilities.credit_cards:,.0f} in credit card debt. "
        ...
        if p.liabilities.credit_cards > 0:
            consumer_debts.append(
                {
                    "name": "Credit Cards",
                    "balance": p.liabilities.credit_cards,
                    "rate": 0.22,
                    "min_payment": max(25.0, p.monthly_debt_payments.credit_cards),
                }
            )
        if p.liabilities.car_loans > 0:
            consumer_debts.append(
                {
                    "name": "Car Loans",
                    "balance": p.liabilities.car_loans,
                    "rate": 0.07,
                    "min_payment": max(25.0, p.monthly_debt_payments.car),
                }
            )
        if p.liabilities.student_loans > 0:
            consumer_debts.append(
                {
                    "name": "Student Loans",
                    "balance": p.liabilities.student_loans,
                    "rate": 0.06,
                    "min_payment": max(25.0, p.monthly_debt_payments.student_loans),
                }
            )
    ...
    def _check_cash_flow(self, p: ClientProfile) -> List[PlanningIssue]:
        ...
        cf = calc.calc_monthly_cash_flow(gm, p.monthly_expenses.total(), p.monthly_debt_payments.total(), tax_rate)
        if cf < 0:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Estimated monthly deficit of ${abs(cf):,.0f}. "
            ...
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Estimated monthly surplus is ${cf:,.0f} "
    ...
    def _check_insurance(self, p: ClientProfile) -> List[PlanningIssue]:
        ...
        if p.gross_annual_income > 0:
            ...
            min_mult = (
                r["life_insurance_dependent_multiple"] if has_dependents else r["life_insurance_recommended_multiple"]
            )
            ...
            if not p.insurance.has_life and has_dependents:
                issues.append(
                    PlanningIssue(
                        ...
                        detail=(
                            "No life insurance coverage detected while the household has dependents. "
                ...
                issues.append(
                    PlanningIssue(
                        ...
                        detail=(
                            f"Current coverage of ${p.insurance.life_coverage_amount:,.0f} "
        ...
        if not p.insurance.has_disability and p.gross_annual_income > 30000:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        "A long-term disability is statistically more likely than early death "
        ...
        if p.age >= 50 and not p.insurance.has_ltc:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"At age {p.age}, it is advisable to evaluate long-term care insurance. "
    ...
    def _check_retirement(self, p: ClientProfile) -> List[PlanningIssue]:
        ...
        if lim["over_limit"]:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Calculated employee contribution of ${annual_contrib:,.0f}/yr "
        ...
        if p.retirement.employer_match_pct > 0 and p.retirement.contribution_rate_pct < p.retirement.employer_match_pct:
            issues.append(
                PlanningIssue(
                    category=IssueCategory.RETIREMENT,
                    severity=IssueSeverity.HIGH,
                    ...
                    detail=(
                        f"Employer matches up to {p.retirement.employer_match_pct:.0f}% of income, "
        ...
        balanced_projection = calc.calc_retirement_projection(
            current_balance, annual_contrib + employer_match, years_to_ret, r["balanced_real_return"], inflation_rate
        )

        # SS benefit reduces the income gap that personal savings must cover
        ss_benefit = calc.estimate_social_security_benefit(p.age, p.total_annual_income(), claiming_age=67)
        ...
        if gap > 0 and years_to_ret > 0:
            ...
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Balanced-scenario projection: ${balanced_projection['corpus_nominal']:,.0f} "
        ...
        if p.age < 67 and ss_benefit > 0:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"Estimated SS benefit at FRA (age 67): ~${ss_benefit:,.0f}/yr "
        ...
        if total_income > roth_end:
            issues.append(
                PlanningIssue(
                    ...
                    detail=(
                        f"For 2024, Roth IRA contributions phase out at "
    ...
    def _check_goals(self, p: ClientProfile) -> List[PlanningIssue]:
        ...
        for goal in p.goals:
            ...
            needed = calc.calc_goal_monthly_savings_needed(inflated_target, 0.0, goal.timeline_months)
            inflation_note = (
                f" (inflated from ${goal.target_amount:,.0f} at 3%/yr)" if inflated_target > goal.target_amount else ""
            )
            if needed > monthly_surplus * 0.50:
                issues.append(
                    PlanningIssue(
                        ...
                        detail=(
                            f"Reaching ${inflated_target:,.0f}{inflation_note} in {goal.timeline_months} months "
