...
def generate_scenarios(profile: ClientProfile) -> List[ScenarioProjection]:
    ...
    for name, cfg in SCENARIO_CONFIGS.items():
        boosted_rate = min(profile.retirement.contribution_rate_pct / 100 + cfg["savings_boost"], 0.30)  # cap at 30%
        ...
        monthly_extra = (
            calc.calc_goal_monthly_savings_needed(extra_needed, 0.0, years_to_ret * 12, cfg["return"] / 12)
            if extra_needed > 0
            else 0.0
        )
        ...
        if gap <= 0:
            ...
            status_txt = (
                f"Projected shortfall of ${gap:,.0f} "
